...
$success = "";
$error_message = "";
$conn = mysqli_connect("localhost","root","","admin_side");
if(!empty($_POST["submit_email"])) {
	...
	if($count>0) {
		...
		if($mail_status == 1) {
			$result = mysqli_query($conn,"INSERT INTO otp_exp(otp,is_expired,create_at) VALUES ('" . $otp . "', 0, '" . date("Y-m-d H:i:s"). "')");
...
if(!empty($_POST["submit_otp"])) {
	$result = mysqli_query($conn,"SELECT * FROM otp_expir WHERE otp='" . $_POST["otp"] . "' AND is_expired!=1 AND NOW() <= DATE_ADD(create_at, INTERVAL 24 HOUR)");
	$count  = mysqli_num_rows($result);
	if(!empty($count)) {
		$result = mysqli_query($conn,"UPDATE otp_exp SET is_expired = 1 WHERE otp = '" . $_POST["otp"] . "'");
		$success = 2;
	} else {
		$success =1;
...
<body>
	...
	?>
<center>
<form name="frmUser" method="post" action="otp.html">
	<div class="tblLogin">
		<?php
		...
		?>
		<i class="fas fa-dog" style="font-size:60px";></i>
		<br></br>
